...
with DAG('analytics', start_date=pendulum.datetime(2025,1,1,tz='UTC'), schedule=[TRANSACTIONS_ASSET], catchup=False, max_active_runs=1, max_active_tasks=2, doc_md='Analytics snapshot DAG.') as dag:
    @task
    ...
    prev=None
    for t in tables:
        op=GlueJobOperator(task_id=f'build_{t}', job_name='sparkify-analytics', script_location='s3://{{ var.value.sparkify_bucket }}/glue-scripts/analytics/glue_script.py', iam_role_name='GlueServiceRole', aws_conn_id=AWS_CONN_ID, script_args={'--SQL_FILE':f'sql/{t}.sql','--TABLE_NAME':t}, wait_for_completion=True)
        if prev: prev >> op
        else: interval >> op
        prev=op
    @task(outlets=[ANALYTICS_ASSET])
    def emit_analytics(): return {'tables': tables}
    prev >> emit_analytics()
