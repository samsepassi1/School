"""Convenience helper to run a single ticket through the compiled workflow."""
...
def run_ticket(app, ticket: dict[str, Any]) -> dict[str, Any]:
    ...
    The ``thread_id`` field controls short-term memory: reusing one resumes
    that conversation, picking a new one starts fresh.
    """
    ...
    logger.info("run_ticket ticket=%s -> escalation=%s",
                ticket["ticket_id"], final_state.get("needs_escalation"))
    return final_state
