"""Short-term + long-term memory for UDA-Hub.

* **Short-term (per-thread)** — a SQLite ``SqliteSaver`` checkpointer persists
  the LangGraph state for each ``thread_id``.

* **Long-term (cross-session)** — a SQLite KV store keyed by
  ...
  ``search()`` runs a substring filter so simple semantic-style lookups work
  out of the box; an embeddings-backed index can drop in by replacing
...
def remember_preference(user_id: str, name: str, value: Any) -> None:
    get_long_term_store().put(("customer", user_id), f"pref:{name}",
