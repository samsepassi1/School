"""Escalation agent — produces a structured handoff for a human owner."""
...
class _EscalationSchema(BaseModel):
    ...
    suggested_team: str = Field(
        description="One of: tier2_support, billing_ops, engineering, trust_safety, concierge"
    )
...
SYSTEM = (
    "You are the Escalation agent. The automated system could not resolve this "
    "ticket. Write a brief handoff note for a human agent. Do NOT promise "
    "outcomes to the customer; instead, acknowledge the issue and set "
...
_TEAM_TO_TEMPLATE = {
    ...
    "concierge":     "Our concierge team will reach out within 1 business day to discuss options.",
}
...
def escalation_node(state: AgentState) -> dict[str, Any]:
    ...
    logger.info("escalation ticket=%s team=%s priority=%s",
                state.get("ticket_id"), decision.suggested_team, decision.priority)

    if state.get("ticket_id"):
        db.append_message(
            state["ticket_id"], role="agent", author="escalation",
            content=f"[ESCALATED to {decision.suggested_team} / {decision.priority}] {decision.summary}",
        )
        db.append_message(state["ticket_id"], role="agent", author="escalation",
                          content=full_reply)
        db.update_ticket_status(state["ticket_id"], "escalated")
        db.upsert_ticket_metadata(state["ticket_id"],
                                  routed_to="escalation",
                                  extra={"escalation": decision_dict})

    log = list(state.get("log", []))
    log.append(f"escalation -> team={decision.suggested_team} priority={decision.priority}")
    return {
        "answer": full_reply,
