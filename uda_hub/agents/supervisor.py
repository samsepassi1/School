...
* ``hydrate_node`` — load the customer's profile, ticket history, and stored
  preferences from long-term memory at the start of every run.

* ``supervisor_router`` — pure routing function used as a conditional edge by
  the LangGraph. Reads classification + retrieval confidence + urgency and
  picks the next agent (resolver vs. escalation).
"""
...
from uda_hub.memory import recall_customer_history, recall_preferences, remember_resolution
...
# --------------------------------------------------------------------------- #
# Hydrate: pull stored context before classification
# --------------------------------------------------------------------------- #
...
def hydrate_node(state: AgentState) -> dict[str, Any]:
    ...
    # Persist the inbound ticket if it is new
...
def supervisor_router(state: AgentState) -> Literal["resolver", "escalation"]:
    """Decide the next node after retrieval has run."""
    ...
    # Furious customer with no clear answer in the KB -> escalate.
    if sentiment == "negative" and confidence < threshold:
        logger.info(
            "router -> escalation (negative sentiment, conf=%.2f < %.2f)",
            confidence, threshold,
        )
        return "escalation"

    # Generic low-confidence -> escalate.
...
def _looks_like_actionable(state: AgentState) -> bool:
    """Cheap heuristic: a billing/account ticket with a verb the resolver can act on
    should still go to the resolver even when KB confidence is so-so."""
    text = f"{state.get('subject','')} {state.get('body','')}".lower()
    actionables = ("refund", "charge", "downgrade", "upgrade", "cancel", "pause")
    classification = state.get("classification") or {}
    if classification.get("category") in {"billing", "account"} and any(a in text for a in actionables):
        return True
    return False


# --------------------------------------------------------------------------- #
# Memory-write step (long-term memory persistence)
# --------------------------------------------------------------------------- #
